...
C       IFFLOW,IFHEAT,IFTRAN,IFNAV
        ...
        READ(9,*,ERR=59)IFSRC   
        READ(9,*,ERR=59)IFCENTRAL
        READ(9,*,ERR=59)IFUPWIND
        READ(9,*,ERR=59)IFTM  
        READ(9,*,ERR=59)IFTE      
        READ(9,*,ERR=59)IFDEALIAS 
        READ(9,*,ERR=59)IFRK4   
        READ(9,*,ERR=59)IFEXP  
        READ(9,*,ERR=59)IFEIG 
        READ(9,*,ERR=59)IFNM   
...
C        Print Out Parameters
     ...
     $      ' ; AXISYMMETRIC$')
      ...
      ELSE IF(CHOICE.EQ.'ALTER PARAMETERS') THEN
         IFALTR=.TRUE.
         GO TO 310  ! temorary misun 2/4/09
         !GO TO 300  Fixme misun 2/4/09
      ENDIF
C
...
1050  CONTINUE
     ...
     $'2 CONDUCT; 2RHOCP'
      ...
      WRITE(10,*)IFSRC,'     IFSRC'
      WRITE(10,*)IFCENTRAL,'     IFCENTRAL'
      WRITE(10,*)IFUPWIND,'     IFUPWIND'
      WRITE(10,*)IFTM,'     IFTM'
      WRITE(10,*)IFTE,'     IFTE'
      WRITE(10,*)IFDEALIAS,'     IFDEALIAS'
      WRITE(10,*)IFRK4,'     IFRK4'
      WRITE(10,*)IFEXP ,'     IFEXP'
      WRITE(10,*)IFEIG ,'     IFEIG'
      WRITE(10,*)IFNM ,'     IFNM'
...
C      ITEM(2)=ITEMON
      DO 20 I=2,12
         ITEMON=ITEM(I)
         IF    (I.EQ.2) THEN
            IF(     IFCEM)ITEMON(25:25)='Y'
            IF(.NOT.IFCEM)ITEMON(25:25)='N'
         ELSEIF(I.EQ.3)THEN
...
C     Round robin for dimensions
      ...
      IF(CHOICE.EQ.'NEKCEM VERSION          Y')IFCEM =.FALSE.
      IF(CHOICE.EQ.'NEKCEM VERSION          N')IFCEM=.TRUE.
...
c-----------------------------------------------------------------------
      SUBROUTINE PREXIT
      INCLUDE 'basics.inc'
      CHARACTER CTEMP*80,CHAR1*1,CHTEMP*3, CB3*3
c      LOGICAL IFMVBD
...
C
      ...
      WRITE(10,'(3I6,''           NEL,NDIM,NELV'')')NELsgn,NDIM,NELV
C
      DO 98 IEL=1,NEL
         IF (IEL.GT.52) LETAPT(IEL) = 'A'
         IF (IFFMTIN) THEN
C           formatted
            WRITE(10,'(A19,I5,A2,I5,A1,A11,I5)')
     $   '            ELEMENT',IEL,' [',NUMAPT(IEL),LETAPT(IEL),
     $   ']    GROUP ',IGROUP(IEL)
            IF(NDIM.EQ.2)THEN
               WRITE(10,'(4G16.8)',ERR=60)(X(IEL,IC),IC=1,4)
               WRITE(10,'(4G16.8)',ERR=60)(Y(IEL,IC),IC=1,4)
            ELSE IF(NDIM.EQ.3)THEN
               WRITE(10,'(4G16.8)',ERR=60)(X(IEL,IC),IC=1,4)
               WRITE(10,'(4G16.8)',ERR=60)(Y(IEL,IC),IC=1,4)
               WRITE(10,'(4G16.8)',ERR=60)(Z(IEL,IC),IC=1,4)
               WRITE(10,'(4G16.8)',ERR=60)(X(IEL,IC),IC=5,8)
               WRITE(10,'(4G16.8)',ERR=60)(Y(IEL,IC),IC=5,8)
               WRITE(10,'(4G16.8)',ERR=60)(Z(IEL,IC),IC=5,8)
            ENDIF
         ELSE
C           Unformatted
            WRITE(11) IGROUP(IEL)
...
C           !!?? NELF DIFFERENT FROM NEL??
     ...
     $               (BC(II,ISIDE,IEL,IFLD),II=1,5)
                  ...
                  CB3= CBC(ISIDE,IEL,IFLD) 
                  ICBC= ICHAR(CB3(1:1)) ! temporary fix: misun 2/4/09
                  !ICBC= ICHAR(CBC(ISIDE,IEL,IFLD)) !original by pff

                  IF(ICBC.GE.97 .AND. ICBC.LE.122 )THEN
C                    Small letter (a--z) for fortran b.c.'s
...
C
      ...
      CPARAM( 2)='PERMIT'
      CPARAM( 3)='PERMEA'
      CPARAM( 4)='RELATIVE PERMIT'
      CPARAM( 5)='RELATIVE PERMEA'
      CPARAM( 6)='GAMMA_P'
      CPARAM( 7)='FREQ_P'
      CPARAM( 8)='GAMMA_P'
      CPARAM( 9)='FREQ_P'
