...
#include "gs.h"
/*
#include "rsb/adj_list.h"
#include "rsb/sparse_matrix.h"
*/
...
#ifdef MLSRC
void ML_XXT_solve(xxt_ADT xxt_handle, int lx, REAL *x, int lb, REAL *b);
...
XXT_new(void)
{
  ...
  /* space for instanciation */
  xxt_handle       = (xxt_ADT)bss_malloc(sizeof(struct xxt_CDT));

  /* I think that any id tag will do */
  xxt_handle->id   = ++n_xxt;

  /* these bits of info aren't here yet ... */
  xxt_handle->info     = NULL;
  xxt_handle->mvi      = NULL;
  xxt_handle->graph    = NULL;

#ifdef MPISRC
  xxt_handle->xxt_comm = NULL;
#endif  

  /* default params ... */
  /* assume full rank matrix */
  xxt_handle->ns=0;

  /* unless directed otherwise we'll reordering interior dofs */
  xxt_handle->reorder_interior = TRUE;

  /* unless directed otherwise we're *not* renumbering proc ids */
  xxt_handle->reorder_interior = FALSE;

  /* is zero a good default? */
  xxt_handle->dim = xxt_handle->level=0;
...
int
#if defined UPCASE
XXT_SOLVE (int *n, REAL *x, REAL *b)
#elif  IBM
xxt_solve (int *n, REAL *x, REAL *b)
#else
xxt_solve_(int *n, REAL *x, REAL *b)
#endif
{
...
**************************************xxt.c***********************************/
int
XXT_solve(xxt_ADT xxt_handle, REAL *x, REAL *b)
{
...
**************************************xxt.c***********************************/
void 
ML_XXT_solve(xxt_ADT xxt_handle, int lx, REAL *sol, int lb, REAL *rhs)
{
...
XXT_stats(xxt_ADT xxt_handle)
{
  ...
  fvals[0]=fvals[1]=fvals[2]=0.0;
  if (xxt_handle->info->nsolves)
  ...
  if (!my_id) 
    {
      ...
      printf("%d :: num   xxt_slv=%d\n",my_id,xxt_handle->info->nsolves);
      printf("%d :: min   xxt_slv=%g\n",my_id,fvals[0]);
      printf("%d :: max   xxt_slv=%g\n",my_id,fvals[1]);
...
int 
#if defined UPCASE
XXT_NULL_SPACE_SIZE (int *ixxt_handle, int *size)
#elif  IBM
xxt_null_space_size (int *ixxt_handle, int *size)
#else
xxt_null_space_size_ (int *ixxt_handle, int *size)
#endif
{
  return(XXT_null_space_size(fhandles[*ixxt_handle], *size));
}
...
Description:  
**************************************xxt.c***********************************/
int XXT_null_space_size(xxt_ADT xxt_handle, int size)
{
  xxt_handle->ns=size;
  return(0);
}
...
int 
#if defined UPCASE
XXT_RENUMBER_PROC  (int *ixxt_handle)
#elif  IBM
xxt_renumber_proc  (int *ixxt_handle)
#else
xxt_renumber_proc_  (int *ixxt_handle)
#endif
{
  return(XXT_renumber_proc(fhandles[*ixxt_handle]));
}
...
**************************************xxt.c***********************************/
int
XXT_renumber_proc(xxt_ADT xxt_handle)
{
  error_msg_fatal("XXT_renumber_proc has yet to be implemented\n");
  return(0);
}
...
int 
#if defined UPCASE
XXT_REORDER_INTERIOR  (int *ixxt_handle, int *flag)
#elif  IBM
xxt_reorder_interior  (int *ixxt_handle, int *flag)
#else
xxt_reorder_interior_  (int *ixxt_handle, int *flag)
#endif
{
  return(XXT_reorder_interior(fhandles[*ixxt_handle],*flag));
}
...
**************************************xxt.c***********************************/
int
XXT_reorder_interior(xxt_ADT xxt_handle, int flag)
{
  xxt_handle->reorder_interior = (flag) ? TRUE : FALSE ;
  return(0);
}
...
int 
#if defined UPCASE
XXT_PUBLISH_GRAPH  (int *ixxt_handle, int *ia, int *ja, int *n)
#elif  IBM
xxx_publish_graph  (int *ixxt_handle, int *ia, int *ja, int *n)
#else
xxx_publish_graph_ (int *ixxt_handle, int *ia, int *ja, int *n)
#endif
{
  return(XXT_publish_graph(fhandles[*ixxt_handle],ia,ja,*n));
}
...
**************************************xxt.c***********************************/
int
XXT_publish_graph(xxt_ADT xxt_handle, int *ia, int *ja, int n)
{
  /*
typedef struct matrix_graph {
  int *ia, *ja, n;
  ...
  */
...
#ifdef DEBUG
  error_msg_warning("XXT_publish_graph() :: start\n");
#endif

  gi = (graph_info*)bss_malloc(sizeof(graph_info));
  gi->n=n;

  /* prob. should be safe and make copy of info */
  nnz = ia[n]-ia[0];
  gi->ia=(int*)bss_malloc((n+2)*INT_LEN);
  gi->ja=(int*)bss_malloc((nnz)*INT_LEN);
  ivec_copy(gi->ia,ia,n+1); ia[n+1]=-1;
  ivec_copy(gi->ja,ja,nnz);
  ...
  if (lb==0)
    ...
    {error_msg_fatal("XXT_publish_graph() :: expected lb of ja to be 0 or 1 not %d\n",lb);}

#ifdef DEBUG
  error_msg_warning("XXT_publish_graph() :: end\n");
#endif
...
int 
#if defined UPCASE
XXT_PUBLISH_DIMENSION  (int *ixxt_handle, int *dim)
#elif  IBM
xxt_publish_dimension  (int *ixxt_handle, int *dim)
#else
xxt_publish_dimension_ (int *ixxt_handle, int *dim)
#endif
{
  return(XXT_publish_dimension(fhandles[*ixxt_handle],*dim));
}
...
**************************************xxt.c***********************************/
int
XXT_publish_dimension(xxt_ADT xxt_handle, int dim)
{
  xxt_handle->dim=dim;
  return(0);
}
...
Check mpi reference manual to det. how to implement fortran interface ...
**************************************xxt.c***********************************/
#ifdef MPISRC
int XXT_set_comm_context(xxt_ADT xxt_handle, MPI_Comm comm)
{
  error_msg_fatal("XXT_set_comm_context() :: not implmented yet\n");
  return(0);
}
#endif
...
xxt_generate(xxt_ADT xxt_handle)
{
  ...
  for (dim=i=j=0;i<m;i++)
      ...
      while (i==segs[dim])
	{
      ...
      col =  (start<end) ? fo[start] : INT_MAX ;
      ...
      if (len>0)
	{
	  ...
	  /* rvec_dump(v+off,len,i,col,"XXT VEC"); */
...
det_separators(xxt_ADT xxt_handle)
{
  ...
  lhs  = (REAL*)bss_malloc(REAL_LEN*m);
  rhs  = (REAL*)bss_malloc(REAL_LEN*m);
  ...
  /* want to hack in rsb here */
  if (xxt_handle->reorder_interior)
    {
      int *ia, *ja, *local_unused, nn, j, *iptr;
      int matrix;
      int *color;
      double *vals, *dptr;

      local_unused = (int *) bss_malloc(INT_LEN * n);


      /* count number in interior = unmarked dofs */
      for (ct=i=0;i<n;i++)
	{
	  if (!used[i]) 
	    {local_unused[ct++] = i;}
      ...
      nn=ct;
      ia = (int *) bss_malloc(INT_LEN * (nn+2));
      ja = (int *) bss_malloc(INT_LEN * (nn+1) * 9);
      vals = (double *) bss_malloc(sizeof(double) * (nn+1) * 9);
      ...
      for (i=0;i<nn;i++)
	{
	  rvec_zero(lhs,n);
	  rvec_zero(rhs,n);
	  lhs[local_unused[i]] = 1.0;
	  do_matvec(xxt_handle->mvi,lhs,rhs);
	  for (j=0;j<nn;j++)
	    {
	      if ((rhs[local_unused[j]]!=0.0))
		{
		  if (ct >= ((nn+1)*9-1))
		    {error_msg_fatal("not enough space for ja\n");}
		  ja[ct++]=j;
		}
	    }
	  ia[i+1] = ct;
	...
	{printf("%.1f ",vals[i]);}
      ...
      j = i - nn;
      if (j&1)
	{error_msg_warning("matrix not symmetric!!\n");}
      else
	{error_msg_warning("matrix symmetric!!\n");}

      SMI_init();
      error_msg_warning("done SMI_init\n");
      matrix = SMI_csc(ia,ja,vals,nn,FULL);
      error_msg_warning("done SMI_csc\n");
      color = (int *) bss_malloc(INT_LEN * nn);
      i = SMI_rsb(matrix,color,log_2_floor(nn));

      ivec_dump(color,nn,i,ia[n]-ia[1],"color");
      error_msg_warning("done SMI_rsb\n");
      fflush(stdout);
      exit(0);
    }
  else
