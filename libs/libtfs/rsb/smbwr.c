...
sm_bandwidth_reduction(int *in_list,int *out_list,int type)
{
  ...
  error_msg_warning("starting sm_bandwidth_reduction\n");
  ...
  if (rt=load_sm(in_list))
    {
      error_msg_warning("success load_sm, who_min = %d\n",who_min);

      in_bw = bw(in_list,out_list,IN);
      error_msg_warning("done bw = %d\n",in_bw);

      /* don't use std cm process if graph is disconnected   */
      if ((rt = dc_graph(out_list)) != DC)
	{
	  error_msg_warning("not disconnected, who_min = %d\n",who_min);
