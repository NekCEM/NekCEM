...
void
dump_adj_row(adj_node_ptr adj_list, int N, int i)
{
  adj_node_ptr ptr;
  
  printf("looking for %8d of %8d:\n",i+1,N);
  fflush(stdout);
  ...
  if (adj_list != NULL)
      ...
      for (i=0;i<N;i++)
	{
	  ...
	  while (ptr != NULL)
		...
		{
		  dump_adj_row(adj_list,N,i);
		  dump_adj_row(adj_list,N,ptr->elmt);
		  fflush(stdout);
		  return(FALSE);
		}
	      ptr = ptr->next;
...
adj_find(adj_node_ptr adj_list, int N, int i, int j, double val)
{
  ...
  while (ptr != NULL)
    {
      if ((ptr->elmt==j)&&(fabs(ptr->val - val) < EPS))
	{return(TRUE);}
