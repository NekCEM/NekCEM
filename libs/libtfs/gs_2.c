/*$Id: gs.c,v 1.5 2001/04/12 20:02:53 balay Exp $*/
/***********************************gs.c***************************************
...
void fgs_init_vec_sz (int *size)
#else
void fgs_init_vec_sz_(int *size)
#endif
{
  gs_init_vec_sz(*size);
}
...
{
  /*  vec_ch = TRUE; */
  /* should be able to set for each one not all */
  vec_sz = size;
  /*printf("gs_init_vec_sz: %i \n",vec_sz);*/
}
...
void  fgs_init_msg_buf_sz  (int *buf_size)
#else
void  fgs_init_msg_buf_sz_ (int *buf_size)
#endif
{
...
#else
int 
fgs_init_ (int *elms, int *nel, int *level)
#endif
{
  gs_id *gsh;
  
  /*printf ("gs_init: %i %i %i \n",*elms,*nel,*level);*/
...
extern void fgs_gop  (int *gs, REAL *vals, char *op)
#else
extern void fgs_gop_ (int *gs, REAL *vals, char *op)
#endif
{
  gs_id *gsh;
...
extern void fgs_gop_vec  (int *gs, REAL *vals, char *op, int *step)
#else
extern void fgs_gop_vec_ (int *gs, REAL *vals, char *op, int *step)
#endif
{
  gs_id *gsh;
	   ...
	   register int step)
{
  ...
  /* check top make sure that segments being requested aren't larger */
  /* then what I reserved earlier ... fix is to allow user to reset  */
  if (step>vec_sz)
    {error_msg_fatal("gs_gop_vec() :: %d > %d!\n",step,vec_sz);}
#endif
