...
s= spatial_matrix(:); slen= length(s); slen=sqrt(slen);
m= reshape(s,slen,slen); 

figure(100);spy(m,'k.');title('Spatial Operator: Strucutre');

e=eigenvalue(:,:); ermax=max(e(:,1));eimax=max(e(:,2));
figure(200);plot(e(:,1),e(:,2),'k.');title('Eigenvalue Distribution (\lambda)');    
